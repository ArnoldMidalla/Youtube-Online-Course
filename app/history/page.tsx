...
export default function HistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      const uniqueIds = Array.from(new Set((data || []).map((h) => h.video_id)));
  ...
  if (history.length === 0) return <div>No watch history yet — play some videos!</div>;
          ...
          return (
            <li key={h.id} className="flex gap-4 p-3 border rounded-lg shadow-sm">
              ...
              <div className="flex-1">
                <div className="font-semibold">{info?.snippet?.title || h.video_id}</div>
                ...
                <div className="text-sm">
                  Progress: {h.progress_percent ?? 0}% ({h.progress_seconds}s / {h.duration_seconds}s)
                </div>
                <div className="text-sm text-gray-500">
